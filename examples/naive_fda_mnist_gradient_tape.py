...
@tf.function
def should_synchronize_models(divergence):
    global threshold
...
@tf.function
def training_step(images, labels, first_batch, last_sync_model):
    increment_counter()
    ...
    updated_last_sync_model = [0.0] * len(mnist_model.trainable_variables)
    ...
    if should_synchronize_models(averaged_divergence) or first_batch:
...
for batch, (images, labels) in enumerate(
    ...
    loss_value, last_sync_model = training_step(images, labels, batch == 0, last_sync_model)
