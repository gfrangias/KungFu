...
def MySynchronousSGDOptimizer(optimizer,
                            barrier=tf.constant(1),
                            nccl=False,
                            nccl_fusion=False,
                            ...
                            use_locking=False,
                            with_keras=False):
    """MySynchronousSGDOptimizer implements the [S-SGD]_ algorithm.
    ...
    """
    my_sync_sgd_algo = _MySynchronousSGD(nccl=nccl,
                                    barrier=barrier,
                                    nccl_fusion=nccl_fusion,
                                    hierarchical_nccl=hierarchical_nccl,
                                    monitor=monitor)
    if with_keras:
        return _create_kungfu_keras_optimizer(optimizer, my_sync_sgd_algo)
    else:
        return _create_kungfu_optimizer(optimizer, my_sync_sgd_algo, name,
                                        use_locking)


class _MySynchronousSGD(_KungFuAlgorithm):
    def __init__(self,
                 barrier=tf.constant(1),
                 nccl=False,
                 nccl_fusion=True,
